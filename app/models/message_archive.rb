class MessageArchive < ActiveRecord::Base
  ...
  def self.getUniqueMessages(user, offset='0')
    ...
    return ActiveRecord::Base.connection.execute('SELECT s.id, body, u.xmpp, r.xmpp FROM message_archives as s JOIN (select max(id) as maxid from message_archives where (receiver_id=' \
                        ...
                        + ' OR sender_id= ' \
                        + user.id.to_s \
                        + ' ) AND sender_id IS NOT NULL AND receiver_id IS NOT NULL group by sender_id) as m ON m.maxid = s.id JOIN  users as u ON u.id=s.sender_id JOIN users r ON r.id=s.receiver_id GROUP BY u.id ORDER BY s.id DESC LIMIT 10 ' \
                        + ' OFFSET ' \
                        + offset).map{ |r|  {id: r[0], body: r[1], sender: r[2], receiver: r[3]}  };
  end
end
