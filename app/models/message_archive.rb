class MessageArchive < ActiveRecord::Base
  ...
  def self.getUniqueMessages(user, minMessageId)
    ...
    return ActiveRecord::Base.connection.execute('SELECT s.id, body, u.xmpp, r.xmpp, u.name, r.name FROM message_archives as s JOIN (select max(id) as maxid from message_archives where (receiver_id=' \
                        + user.id.to_s \
                        + ' OR sender_id= ' \
                        + user.id.to_s \
                      ...
                      ).map { |r| {id: r[0], body: r[1], sender: r[2], receiver: r[3], sender_name: r[4], receiver_name: r[5]} }
  end
end
